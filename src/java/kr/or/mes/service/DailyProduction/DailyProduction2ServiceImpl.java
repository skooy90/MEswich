...
public class DailyProduction2ServiceImpl implements DailyProduction2Service {
	...
	public int updateDailyProductionStatus(DailyProduction2DTO dailyProduction) {
		...
		// 금일 생산계획 상태 업데이트 성공 시 전체 생산계획 상태도 업데이트
		if (result > 0) {
			// 금일 생산계획 정보 조회하여 parent_lot_number 가져오기
			DailyProduction2DTO dailyInfo = dailyDAO.selectDailyProductionById(dailyProduction.getDailyPlanId());
			if (dailyInfo != null) {
				String parentLotNumber = dailyInfo.getParentLotNumber();
				String parentStatus = determineParentStatus(dailyProduction.getStatus());

				if (parentStatus != null) {
					Production2DTO parentProduction = new Production2DTO();
					parentProduction.setLotNumber(parentLotNumber);
					parentProduction.setStatus(parentStatus);
					parentProduction.setUpdatedBy("SYSTEM");

					productionService.updateStatus(parentProduction);
				}
			}
		}
	...
	/**
	 * 금일 생산계획 상태에 따른 전체 생산계획 상태 결정
	 * 
	 * @param dailyStatus 금일 생산계획 상태
	 * @return 전체 생산계획 상태
	 */
	private String determineParentStatus(String dailyStatus) {
		if ("work".equals(dailyStatus) || "quality".equals(dailyStatus)) {
			return "IN_PROGRESS";
		} 
