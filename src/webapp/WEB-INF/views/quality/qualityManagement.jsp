...
<body>
    ...
    <main class="main-content">
        ...
        <div class="search-area">
            <form class="search-form" method="get" action="/mes/quality/search">
        ...
        <div class="list-area">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${not empty waitingList}">
                                <c:forEach var="quality" items="${waitingList}">
                                    <tr>
                                        <td>${quality.inspectionNo}</td>
                                        <td>${quality.lotNumber}</td>
                                        <td>${quality.productName}</td>
                                        <td>${quality.actualQty != null ? quality.actualQty : 0}개</td>
                                        <td>${quality.inspectorName != null ? quality.inspectorName : '미배정'}</td>
                                        <td><fmt:formatDate value="${quality.createdDate}" pattern="yyyy-MM-dd" /></td>
                                        <td>
                                            <button class="action-btn btn-warning" onclick="startInspection('${quality.inspectionNo}')">검사시작</button>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </c:when>
            ...
            <div id="inspecting-tab" class="tab-content" style="display: none;">
                ...
                <table class="quality-list">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${not empty inspectingList}">
                                <c:forEach var="quality" items="${inspectingList}">
                                    <tr>
                                        <td>${quality.inspectionNo}</td>
                                        <td>${quality.lotNumber}</td>
                                        <td>${quality.productName}</td>
                                        <td>${quality.actualQty != null ? quality.actualQty : 0}개</td>
                                        <td>${quality.inspectorName}</td>
                                        <td><fmt:formatDate value="${quality.updatedDate}" pattern="yyyy-MM-dd" /></td>
                                        <td>
                                            <button class="action-btn btn-success" onclick="completeInspection('${quality.inspectionNo}')">검사완료</button>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </c:when>
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${not empty completedList}">
                                <c:forEach var="quality" items="${completedList}">
                                    <tr>
                                        <td>${quality.inspectionNo}</td>
                                        <td>${quality.lotNumber}</td>
                                        <td>${quality.productName}</td>
                                        <td>${quality.goodQty != null ? quality.goodQty : 0}개</td>
                                        <td>${quality.defectQty != null ? quality.defectQty : 0}개</td>
                                        <td>${quality.inspectorName}</td>
                                        <td><fmt:formatDate value="${quality.inspectionDate}" pattern="yyyy-MM-dd" /></td>
                                        <td>
                                            <c:if test="${quality.status == 'PASS'}">
                                                <button class="action-btn btn-info" onclick="transferToInventory('${quality.inspectionNo}')">재고전달</button>
                                            </c:if>
                                            <a href="/mes/quality/detail?inspectionNo=${quality.inspectionNo}" class="action-btn btn-primary" style="text-decoration: none; display: inline-block; text-align: center;">상세보기</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </c:when>
    ...
    <script>
        ...
        function startInspection(inspectionId) {
            ...
            if (inspector && inspector.trim() !== '') {
                if (confirm('검사를 시작하시겠습니까?')) {
                    fetch('/mes/quality/startInspection', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: 'inspectionNo=' + encodeURIComponent(inspectionId) + 
                              '&inspectorId=' + encodeURIComponent(inspector) +
                              '&inspectorName=' + encodeURIComponent(inspector)
                    })
        ...
        document.getElementById('inspectionForm').addEventListener('submit', function(e) {
            ...
            // 서버로 검사 결과 전송
            fetch('/mes/quality/completeInspection', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: 'inspectionNo=' + encodeURIComponent(currentInspectionId) + 
                      '&goodQty=' + encodeURIComponent(goodQty) +
                      '&defectQty=' + encodeURIComponent(defectQty) +
                      '&defectReason=' + encodeURIComponent(defectReason)
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
        ...
        function transferToInventory(inspectionId) {
            if (confirm('재고관리로 완품을 전달하시겠습니까?')) {
                fetch('/mes/quality/transferToInventory', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
